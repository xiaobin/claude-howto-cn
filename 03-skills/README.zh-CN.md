...
有关详细检查清单，参见 [templates/review-checklist.md](templates/review-checklist.md)。
```
...
有关代码异味目录，参见 [references/code-smells.md](references/code-smells.md)。
有关重构技术，参见 [references/refactoring-catalog.md](references/refactoring-catalog.md)。
```
...
- 使用**相对路径**从 `SKILL.md` 引用额外文件（例如 `[API reference](references/api-spec.md)`）。
...
- 完整的 API 详情，参见 [reference.md](reference.md)
